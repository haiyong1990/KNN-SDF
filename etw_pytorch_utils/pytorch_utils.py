...
class ResSharedMLP(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.args = args
        self.fc_dx = SharedMLP(args, bn, activation, preact, first, name)
        size_in, size_out = args[0], args[-1]
        if size_in == size_out:
            self.shortcut = None
        else:
            self.shortcut = nn.Conv2d(size_in, size_out, 1, 1, bias=False)

        # for ii in range(len(self.fc_dx)):
        #    nn.init.zeros_(self.fc_dx[ii].conv_unit.weight)

    def forward(self, inputs):
        x = inputs
        dx = self.fc_dx(x)

        # print(x.shape, dx.shape, self.args)
