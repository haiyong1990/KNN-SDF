...
def setup_GPU(ngpu=1, xsize=5000):
    ## setup GPU
...
from im2mesh.utils.io import export_pointcloud, export_pointcloud_details
...
cfg["data"]["sample_dense_surf"] = True
...
if cfg['test']['model_file'].endswith(".pt"):
...
test_loader = torch.utils.data.DataLoader(
    dataset, batch_size=1, num_workers=2, shuffle=False)
...
# for it, data in enumerate(test_loader):
    ...
    # print("gen_dir: ", generation_vis_dir)
    ...
    # Also copy ground truth, TODO: generate watertight meshes
    ...
    if cfg['generation']['copy_input']:
        ...
        normals = data['inputs.normals'].squeeze(0).cpu().numpy()
        export_pointcloud_details(inputs, normals, 0.03,
                inputs_path.replace(".ply", "_inputs.ply"), False)
        # export_pointcloud(inputs, inputs_path.replace(".ply", "_inputs.ply"), False)
        out_file_dict['in'] = inputs_path
        points = data['inputs.points'].squeeze(0).cpu().numpy()
        export_pointcloud(points, inputs_path.replace(".ply", "_gnd.ply"), False)
        if generate_mesh and mesh is not None:
            mesh.export(inputs_path.replace(".ply", "_pred.off"))

        if generate_pointcloud and pointcloud is not None:
            pass
            # export_pointcloud(pointcloud, inputs_path.replace(".ply", "_pcd.ply"))
            # points = data['points_iou'].cpu().numpy().reshape(-1,3)
            # occ = data['points_iou.occ'].cpu().numpy().reshape(-1,).astype(np.bool)
            # points = points[occ, :]
            # export_pointcloud(points, inputs_path.replace(".ply", "_train.ply"), False)
            # points = generator.generate_grids(data)
            # export_pointcloud(points, inputs_path.replace(".ply", "_grid.ply"), False)
