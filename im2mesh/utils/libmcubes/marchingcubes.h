...
template<typename coord_type, typename vector3, typename formula1, typename formula2>
void marching_cubes_voxels_correct(
        const vector3 lower, 
        const int numx, const int numy, const int numz, int numv, 
        const double dx, const double dy, const double dz, 
        formula1 f1, formula2 f2, double isovalue, 
        std::vector<double>& vertices, std::vector<size_t>& polygons)
{
    // 0.0, 0.5, [1.0], 1.5, [2.0], 2.5
    // low: 0.0, dd: 1.0, num: 2.0
    // The algorithm is adopted from regular grids, we simply extract voxels in the same yz plane, and excute marching cube to get geometry. 
    // The algorithm is not optimized, so it may be slow. 
    // TODO: use smarter indexing algorithm to avoid duplicate vertices
    // TODO: double check the index and numx/y/z, index starts from 1.0, num should be || + 1.
    using namespace private_;
    ...
    for(int i=0; i<numx; ++i)
    {
        ...
        for(int iv=0; iv<numv; ++iv)
        {
            double coords[3];
            f2(iv, coords);
            if( fabs(coords[0]-x) > eps )
                continue;

            x =  coords[0] - dx/2 ;
            coord_type x_dx = coords[0] + dx/2 ;   
            coord_type y = coords[1] - dy/2;
            coord_type y_dy =  coords[1] + dy/2;
            coord_type z = coords[2] - dz/2;
            coord_type z_dz = coords[2] + dz/2;
            int j = int((coords[1] - lower[1])/dy) - 1;
            int k = int((coords[2] - lower[2])/dz) - 1;
            // assert(j>=0 && k>=0);
...
template<typename coord_type, typename vector3, typename formula1, typename formula2>
void marching_cubes_voxels(
        const vector3 lower, 
        const int numx, const int numy, const int numz, const int numv, 
        const double dx, const double dy, const double dz, 
        formula1 f1, formula2 f2, double isovalue, 
        std::vector<double>& vertices, std::vector<size_t>& polygons)
{
    ...
    for(int iv=0; iv<numv; ++iv)
    {
        ...
        // WARNNING: NOTE THAT THIS VERSION GENERATES DUPLICATE VERTICES.
