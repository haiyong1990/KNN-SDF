...
trilinear_devoxelize_forward(const int r, const bool is_training,
  ...
  if (is_training) {
    ...
    trilinear_devoxelize(b, c, n, r, r2, r3, true, coords.data<float>(),
                         features.data<float>(), inds.data<int>(),
                         wgts.data<float>(), outs.data<float>());
    ...
    trilinear_devoxelize(b, c, n, r, r2, r3, false, coords.data<float>(),
                         features.data<float>(), inds.data<int>(),
                         wgts.data<float>(), outs.data<float>());
    return {outs, inds, wgts};
  }
...
at::Tensor trilinear_devoxelize_backward(const at::Tensor grad_y,
  ...
  trilinear_devoxelize_grad(b, c, n, r3, indices.data<int>(),
                            weights.data<float>(), grad_y.data<float>(),
                            grad_x.data<float>());
  return grad_x;
}
