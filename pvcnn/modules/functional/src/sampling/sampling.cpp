...
at::Tensor gather_features_forward(at::Tensor features, at::Tensor indices) {
  ...
  gather_features(b, c, n, m, features.data<float>(),
                  indices.data<int>(), output.data<float>());
  return output;
}
...
at::Tensor gather_features_backward(at::Tensor grad_y, at::Tensor indices,
  ...
  gather_features_grad(b, c, n, indices.size(1), grad_y.data<float>(),
                       indices.data<int>(), grad_x.data<float>());
  return grad_x;
}
...
at::Tensor furthest_point_sampling_forward(at::Tensor coords,
  ...
  furthest_point_sampling(b, n, num_samples, coords.data<float>(),
                          distances.data<float>(), indices.data<int>());
  return indices;
}
