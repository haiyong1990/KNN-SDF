...
std::vector<at::Tensor> avg_voxelize_forward(const at::Tensor features,
  ...
  avg_voxelize(b, c, n, r, r2, r3, coords.data<int>(),
               features.data<float>(), ind.data<int>(),
               cnt.data<int>(), out.data<float>());
  return {out, ind, cnt};
}
...
at::Tensor avg_voxelize_backward(const at::Tensor grad_y,
  ...
  avg_voxelize_grad(b, c, n, s, indices.data<int>(), cnt.data<int>(),
